...
Ещё укажи поле indie — true, если новость про одиночного разработчика или
крошечную команду и экономику их продукта. Формат продукта любой: мобильное
приложение, веб-сервис, SaaS, телеграм- или дискорд-бот, расширение браузера,
десктопная утилита, API, платный плагин или шаблон.
Подходят: сколько продукт зарабатывает, выручка или MRR, доходы в App Store
и других сторах, запуск без инвестиций, разбор монетизации, отчёт о доходах,
поиск первых платящих пользователей, история провала или закрытия проекта.
Ставь true только при наличии конкретики. Мотивационные посты, курсы и
«как заработать в интернете» — это false и низкий балл.
...
def split_digest(
    ...
    """Делит отобранное на три раздела.

    «Главное» — просто лучшее по баллу, тема любая. Два тематических раздела
    добираются из того, что в первый не поместилось: они добавляют объём,
    не отбирая места у сильных новостей и не дублируя их.

    Инди разбирается раньше стартапов: тема у́же, и если новость подходит
    обеим, ей место именно там. Иначе весь инди-контент осел бы в разделе
    про стартапы, а третий раздел остался бы пустым.

    Делить жёстко по теме пробовали — получалось хуже: «Главное» усыхало до
    пяти пунктов, а порядок чтения ломался, потому что крупные сделки уезжали
    во второй раздел просто по признаку темы.
    """
    ranked = sorted(items, key=lambda i: i.score, reverse=True)
    ...
    indie = [i for i in ranked if i.is_indie and i.url not in chosen]
    indie = indie[:indie_size]
    chosen.update(i.url for i in indie)
